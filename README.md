...
- 整刊跨页阅读：发布前把横向跨页转换成独立的左、右单页，奇偶页均可稳定阅读
...
| `content_comments` | 社刊、社刊文章与成员作品点评 | 对应内容公开时可读 | 正式账号 |
| `content_ratings` | 社刊、社刊文章与成员作品五星评分 | 对应内容公开时可读 | 正式账号，每项一条 |
...
- DOC：旧版二进制 Word 格式不支持可靠的浏览器解析，阅读器会提示先转换为 PDF 或 DOCX。
- 网页不提供 PDF 直接下载或“新窗口打开”入口，整刊、分篇与成员作品统一通过站内阅读器查看；阅读器打开时会拦截右键菜单和浏览器保存快捷键。
- GitHub Pages 中的社刊 PDF 属于公开静态资源。移除下载入口只能阻止普通界面操作，不能构成 DRM；浏览器既然能读取内容，熟悉开发工具的用户仍可能取得文件数据。若需更严格的访问控制，应把社刊迁入私有 Storage，并结合账号权限和短时签名链接。
