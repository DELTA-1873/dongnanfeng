...
def write_reader_issue(reader: PdfReader, output: Path):
    ...
    writer = PdfWriter()
    for physical_number, source_page in enumerate(reader.pages, start=1):
        width = float(source_page.mediabox.width)
        height = float(source_page.mediabox.height)
        is_spread = width > height * 1.15 and physical_number != 3
        if is_spread:
...
def main():
    ...
    write_reader_issue(reader, ISSUE_DIR / "dongnanfeng-2025.pdf")
